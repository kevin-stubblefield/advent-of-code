...
func moveCrate(stacks [][]byte, numberToMove, fromStack, toStack int) [][]byte {
	...
	values := stacks[fromStack][len(stacks[fromStack])-numberToMove : len(stacks[fromStack])]
	stacks[fromStack] = stacks[fromStack][:len(stacks[fromStack])-numberToMove]
	stacks[toStack] = append(stacks[toStack], values...)
